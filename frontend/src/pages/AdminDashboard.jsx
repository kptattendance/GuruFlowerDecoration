...
export default function AdminDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      const [statsRes, servicesRes, eventsRes] = await Promise.all([
        fetch(
          "https://guru-flower-decoration-backend.vercel.app/api/admin/stats"
        ),
        fetch(
          "https://guru-flower-decoration-backend.vercel.app/api/admin/services"
        ),
        fetch("https://guru-flower-decoration-backend.vercel.app/api/events"),
      ]);
  ...
  const handleEventImageUpload = async (e) => {
    ...
    try {
      const res = await fetch(
        "https://guru-flower-decoration-backend.vercel.app/api/upload",
        {
          method: "POST",
          body: formData,
        }
  ...
  const toggleItem = async (id, availability, isEvent = false) => {
    try {
      const endpoint = isEvent
        ? `https://guru-flower-decoration-backend.vercel.app/api/events/${id}`
        : `https://guru-flower-decoration-backend.vercel.app/api/admin/services/${id}`;
  ...
  const deleteItem = async (id, isEvent = false) => {
    ...
    try {
      const endpoint = isEvent
        ? `https://guru-flower-decoration-backend.vercel.app/api/delete/events/${id}` // ✅ FIXED
        : `https://guru-flower-decoration-backend.vercel.app/api/admin/services/${id}`;
            ...
            <div className="admin-card">
              ...
              {loading ? (
                ...
                <div className="empty-state">
                  ...
                  <p>
                    Add your first{" "}
                    {activeTab === "services" ? "service" : "event"}
                  </p>
                </div>
              ) : (
                <div className="services-list">
                  {(activeTab === "services" ? services : events).map(
                    (item) => (
                      <div key={item._id} className="service-item">
                        {activeTab === "events" && item.image && (
                          <div className="service-image-small">
                            <img
                              src={`https://guru-flower-decoration-backend.vercel.app/uploads/${item.image}`}
                              alt={item.title}
                            />
                          </div>
                        )}
                        ...
                        <div className="service-actions">
                          ...
                          <button
                            ...
                            onClick={() =>
                              deleteItem(item._id, activeTab === "events")
                            }
                          >
                            Delete
                          </button>
